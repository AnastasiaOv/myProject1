package ru.mephi.qualityManagement.web.meal;
...
import ru.mephi.qualityManagement.to.UserMealWithExceed;
...
public class UserMealAjaxController extends AbstractMealController {
    ...
    public List<UserMealWithExceed> filterWithExceed(DateTimeFilter filter) {
        return filterWithExceed(super.getBetween(TimeUtil.startDateTime(filter.getStartDate()), TimeUtil.endDateTime(filter.getEndDate())),
                TimeUtil.toTime(filter.getStartTime(), LocalTime.MIN), TimeUtil.toTime(filter.getEndTime(), LocalTime.MAX));
    }
